...
function BodyReader(options) {
    ...
    function readXmlElements(elements) {
        ...
        // if (element.name.endsWith("ins")) {
            // console.log({ elements, cr })
        // }
    ...
    function readXmlElement(element) {
        if (element.type === "element") {
            ...
            if (handler) {
                const resp = handler(element);
                // if (element.name.endsWith("del") || element.name.endsWith("ins"))
                //     console.log({ elementName: element.name, elementType: element.type, ...resp  })
                return resp;
    ...
    function readSuggestionProperties(element) {
        return readSuggestionStyle(element).map(function(style) {
            return {
    ...
    function readSuggestionStyle(element) {
        return readStyle(element, "w:pStyle", "Suggestion", styles.findParagraphStyleById);
    }
    ...
    function readStyle(element, styleTagName, styleType, findStyleById, ) {
        ...
        if (element.name.endsWith('ins'))
            console.log({ element, styleTagName, styleType, findStyleById })
    ...
    var xmlElementReaders = {
        "w:p": function(element) {
            const paraIdKeyArr = Object.keys(element.attributes).filter(x => x.endsWith("paraId")) || [];
            const paraIdKey = paraIdKeyArr[0];
            ...
            if (isDeleted) {
                ...
                const pr = ReadResult.map(
                    readParagraphProperties(paragraphPropertiesElement),
                    readXmlElements(childrenXml),
                    function(properties, children) {
                        return new documents.Paragraph(children, {...properties, paraId: element.attributes[paraIdKey]});
                    }
                ).insertExtra();
                console.log({ type: "para", pr })
                return pr;
            }
        },
        ...
        "w:ins": (element) => {
            const pr = ReadResult.map(
                readSuggestionProperties(element),
                readXmlElements(element.children),
                function(properties, children) {
                    return new documents.Suggestion('insert', children, properties);
                }
            ).insertExtra();
            console.log({suggestion: 'ins', pr})
            return pr;
            // return readChildElements(element).map((elemTemp) => {
            //     console.log({ element, elemTemp })
            //     return elemTemp;
            // })
        },
        "w:del": readChildElements,
