...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options = {}) {
        const no = { ...options, paraId:element.paraId };
        const p = htmlPathForParagraph(element, messages).wrap(function() {
            var content = convertElements(element.children, messages, no);
            console.log({content: content[0], no})
            ...
            p2.tag.attributes = no
            return p2;
        });
        console.log({p: p[0].tag})
        return p;
    }
    ...
    function convertComment(referencedComment, messages, options) {
        ...
        var body = convertElements(comment.body, messages, options).concat([
            Html.nonFreshElement("p", options, [
                Html.text(" "),
    ...
    var elementConverters = {
        ...
        "text": function(element, messages, options = {}) {
            return [Html.text(element.value, { type: element.type, paraId: element.paraId, ...options })]
        },
        "tab": function(element, messages, options) {
            return [Html.text("\t", { type: element.type, paraId: element.paraId, ...options })];
        },
